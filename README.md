...
举个具体的例子：直接让模型记"买车花了 20 万"，它会归到"办公费"——
因为模型不理解"买车是资产购置（借固定资产）、不是费用"这个会计规则。
后果是利润被虚减 20 万，而且**不报错、不易察觉**。
...
**所以在别的地方一个 Agent 都没用**——记账、审核、结账全是确定性规则：
...
当前版本已完成完整的会计循环与内控机制。下一步计划深化的方向：

- **逐笔核销与账龄分析**：收款时指定冲抵哪几笔应收，实现精确的应收账款账龄
- **多账套隔离**：一套系统管理多家企业的账，按企业隔离数据（代账公司场景）
- **完整纳税申报**：从当前的所得税估算扩展为增值税及附加税的完整申报
